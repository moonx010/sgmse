...
# pylint: skip-file
"""
NCSN++ with noise reference conditioning.

This extends the ncsnpp_v2 backbone to accept an additional noise embedding
that conditions the score estimation. The noise embedding is injected through
the time embedding pathway using either additive or FiLM-style conditioning.
"""
...
@BackboneRegistry.register("ncsnpp_v2_cond")
class NCSNpp_v2_NoiseCond(nn.Module):
    """
    NCSN++ model with noise reference conditioning.

    Extends ncsnpp_v2 to accept a noise embedding z_r that conditions
    the score network. The conditioning is applied through the time
    embedding path.
    """
    ...
    def __init__(
        ...
        **unused_kwargs
        ...
        self.noise_cond_type = noise_cond_type
        ...
        # Noise embedding projection
        self.noise_embed_dim = noise_embed_dim
        temb_dim = nf * 4

        if noise_cond_type == 'add':
            # Simple additive conditioning: temb = temb + proj(z_r)
            self.noise_proj = nn.Sequential(
                nn.Linear(noise_embed_dim, temb_dim),
                nn.SiLU(),
                nn.Linear(temb_dim, temb_dim),
            )
        elif noise_cond_type == 'film':
            # FiLM conditioning: temb = gamma * temb + beta
            self.noise_proj_gamma = nn.Sequential(
                nn.Linear(noise_embed_dim, temb_dim),
                nn.SiLU(),
                nn.Linear(temb_dim, temb_dim),
            )
            self.noise_proj_beta = nn.Sequential(
                nn.Linear(noise_embed_dim, temb_dim),
                nn.SiLU(),
                nn.Linear(temb_dim, temb_dim),
            )
            # Initialize to identity transform
            nn.init.ones_(self.noise_proj_gamma[-1].weight)
            nn.init.zeros_(self.noise_proj_gamma[-1].bias)
            nn.init.zeros_(self.noise_proj_beta[-1].weight)
            nn.init.zeros_(self.noise_proj_beta[-1].bias)
    ...
    def forward(self, x, y, t, z_r=None):
        """
        Forward pass with optional noise reference conditioning.

        Args:
            x: Corrupted signal x_t [B, 1, F, T] (complex)
            y: Noisy observation [B, 1, F, T] (complex)
            t: Time step [B]
            z_r: Noise embedding [B, noise_embed_dim] (optional)
        ...
        if z_r is not None:
            if self.noise_cond_type == 'add':
                noise_emb = self.noise_proj(z_r)
                temb = temb + noise_emb
            elif self.noise_cond_type == 'film':
                gamma = self.noise_proj_gamma(z_r)
                beta = self.noise_proj_beta(z_r)
                temb = gamma * temb + beta
