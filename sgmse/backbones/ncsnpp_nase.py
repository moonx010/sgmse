# coding=utf-8
"""
NCSN++ with NASE-style input addition conditioning.

This backbone injects noise embedding directly into the input features
(early fusion) rather than through time embedding (late fusion).

Reference: NASE found that simple addition outperforms FiLM and cross-attention.
"""
...
@BackboneRegistry.register("ncsnpp_nase")
class NCSNpp_NASE(nn.Module):
    """
    NCSN++ with NASE-style noise conditioning.

    Key difference from ncsnpp_v2_cond:
    - Noise embedding is added to early features (input fusion)
    - Not added through time embedding pathway

    This follows NASE's finding that simple addition works best.
    """
    ...
    def __init__(
        ...
        **unused_kwargs
        ...
        # Noise embedding projection (NASE-style: project to first layer feature dim)
        self.noise_embed_dim = noise_embed_dim
        self.noise_proj = nn.Sequential(
            nn.Linear(noise_embed_dim, nf),
            nn.SiLU(),
            nn.Linear(nf, nf),
        )
    ...
    def forward(self, x, y, t, z_r=None):
        """
        Forward pass with NASE-style noise conditioning.
        ...
        # First conv layer
        h = modules[m_idx](x)  # [B, nf, F, T]
        m_idx += 1

        # NASE-style: Add noise embedding to first layer features
        if z_r is not None:
            noise_emb = self.noise_proj(z_r)  # [B, nf]
            # Broadcast and add to spatial features
            h = h + noise_emb[:, :, None, None]
        ...
        return torch.complex(h[:, 0:1], h[:, 1:2])
