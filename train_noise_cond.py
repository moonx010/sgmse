"""
Training script for noise-conditioned SGMSE+.

This script trains the NoiseCondScoreModel which extends SGMSE+ with
noise reference conditioning. The model learns to enhance speech
conditioned on a short noise-only reference signal.

Usage:
    python train_noise_cond.py --base_dir /path/to/data --backbone ncsnpp_v2_cond

For PoC experiments with default settings:
    python train_noise_cond.py --base_dir /path/to/data --backbone ncsnpp_v2_cond \
        --ref_num_frames 32 --noise_embed_dim 512 --noise_cond_type add
"""
...
from sgmse.data_module_cond import NoiseCondSpecsDataModule
...
if __name__ == '__main__':
    ...
    for parser_ in (base_parser, parser):
        parser_.add_argument("--backbone", type=str,
                            ...
                            default="ncsnpp_v2_cond",
                            help="Backbone network (default: ncsnpp_v2_cond)")
        parser_.add_argument("--sde", type=str,
                            choices=SDERegistry.get_all_names(),
                            default="ouve",
                            help="SDE type (default: ouve)")
        ...
        parser_.add_argument("--log_dir", type=str, default="logs",
                            help="Directory to save logs.")
        parser_.add_argument("--save_ckpt_interval", type=int, default=50000,
                            help="Save checkpoint interval.")

    temp_args, _ = base_parser.parse_known_args()

    # Add specific args for model, trainer, SDE, and backbone
    ...
    NoiseCondScoreModel.add_argparse_args(
        parser.add_argument_group("NoiseCondScoreModel",
                                  description=NoiseCondScoreModel.__name__))
    ...
    data_module_cls = NoiseCondSpecsDataModule
    ...
    # Initialize model
    model = NoiseCondScoreModel(
        backbone=args.backbone,
        sde=args.sde,
        data_module_cls=data_module_cls,
        **{
            **vars(arg_groups['NoiseCondScoreModel']),
            **vars(arg_groups['SDE']),
    ...
    if logger is not None:
        ...
        if args.num_eval_files:
            checkpoint_callback_pesq = ModelCheckpoint(
                dirpath=join(args.log_dir, str(logger.version)),
                save_top_k=1, monitor="pesq", mode="max",
                filename='{epoch}-{pesq:.2f}'
            )
            checkpoint_callback_si_sdr = ModelCheckpoint(
                dirpath=join(args.log_dir, str(logger.version)),
                save_top_k=1, monitor="si_sdr", mode="max",
